...
df=pd.read_csv('result_tables/13_go_logistic_models.csv')

# GO terms were messed up by R - replace . with :
df['GO_id']=df.GO_id.str.replace('.', ':')
...
df=df[['GO_id', 'control', 'case_w', 'case_wo', 'control_w', 'control_wo', 'cc_est', 'cc_err', 'cc_p', 'cc_bh_fdr', 'cc_posneg',
...
# Save to file
df.to_csv('result_tables/14_parsed_go_model.csv', index=False)
